...
Pixel Shader::getter_pixel(int x, int y) { return map[x][y] ; } 

std::vector<Pixel> Shader::Distance_draw_color(std::vector<Point> site_list) {
    double temp = 1000000;
    double dis ;
    size_t index ;
    Pixel pixel ; 
...
double Shader::distance(Point p, int x, int y) {
    double xx = p.x()-x ;
    double yy = p.y()-y ;
    return (xx * xx) + (yy * yy) ;
} // distance()

std::vector<Pixel> Shader::BFS_draw_color(std::vector<Point> site_list) {
    int x, y ;
    int _color = 1 ;
    std::vector<Pixel> color_list ; 
    Pixel n_pixel ;
...
void Shader::bfs(Point p, int color) {
    int x = p.x() ;
    int y = p.y() ;
    int x_index = x-1 ;
    int y_index = y-1 ;
